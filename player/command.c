...
static int mp_property_amcfy_audio_levels(void *ctx, struct m_property *p,
                                          int action, void *arg)
{
    MPContext *mpctx = ctx;
    ...
    case M_PROPERTY_GET: {
        ...
        for (int n = 0; n < AMCFY_AUDIO_LEVEL_COUNT; n++)
            node_array_add(&node, MPV_FORMAT_DOUBLE)->u.double_ =
                mpctx->amcfy_audio_levels[n];
        *(struct mpv_node *)arg = node;
        return M_PROPERTY_OK;
    }
    }
...
static const struct m_property mp_properties_base[] = {
    ...
    {"amcfy-audio-levels", mp_property_amcfy_audio_levels},
