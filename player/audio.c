...
static double amcfy_sample_to_level(uint8_t *data, int format)
{
    switch (af_fmt_from_planar(format)) {
    case AF_FORMAT_U8:
        return fabs(((double)*(uint8_t *)data - 128.0) / 128.0);
    case AF_FORMAT_S16:
        return fabs((double)*(int16_t *)data / 32768.0);
    case AF_FORMAT_S32:
        return fabs((double)*(int32_t *)data / 2147483648.0);
    case AF_FORMAT_S64:
        return fabs((double)*(int64_t *)data / 9223372036854775808.0);
    case AF_FORMAT_FLOAT:
        return fabs((double)*(float *)data);
    case AF_FORMAT_DOUBLE:
        return fabs(*(double *)data);
    default:
        return 0.0;
    }
}

static void amcfy_update_audio_levels(struct MPContext *mpctx,
                                      struct mp_aframe *af)
{
    int64_t now = mp_time_ns();
    if (now - mpctx->amcfy_audio_levels_update_ns < MP_TIME_MS_TO_NS(33))
        return;

    int format = mp_aframe_get_format(af);
    if (!af_fmt_is_pcm(format) || af_fmt_is_spdif(format))
        return;

    int samples = mp_aframe_get_size(af);
    int channels = mp_aframe_get_channels(af);
    int bytes = af_fmt_to_bytes(format);
    uint8_t **planes = mp_aframe_get_data_ro(af);
    if (!samples || !channels || !bytes || !planes)
        return;

    bool planar = af_fmt_is_planar(format);
    int bucket_samples = MPMAX(1, samples / AMCFY_AUDIO_LEVEL_COUNT);

    for (int bucket = 0; bucket < AMCFY_AUDIO_LEVEL_COUNT; bucket++) {
        int start = bucket * bucket_samples;
        int end = bucket == AMCFY_AUDIO_LEVEL_COUNT - 1
            ? samples
            : MPMIN(samples, start + bucket_samples);
        double peak = 0.0;

        for (int n = start; n < end; n++) {
            for (int ch = 0; ch < channels; ch++) {
                uint8_t *sample = planar
                    ? planes[ch] + (size_t)n * bytes
                    : planes[0] + ((size_t)n * channels + ch) * bytes;
                peak = MPMAX(peak, amcfy_sample_to_level(sample, format));
            }
        }

        // Amcfy: 只做基础峰值桶，复杂 FFT 留给后续确认需要时再加。
        mpctx->amcfy_audio_levels[bucket] = MPMIN(1.0, peak);
    }

    mpctx->amcfy_audio_levels_update_ns = now;
    mp_notify_property(mpctx, "amcfy-audio-levels");
}
...
static void ao_process(struct mp_filter *f)
{
    ...
    if (frame.type == MP_FRAME_AUDIO) {
        ...
        amcfy_update_audio_levels(mpctx, af);
